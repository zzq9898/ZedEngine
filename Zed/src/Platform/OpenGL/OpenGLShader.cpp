...
namespace Zed{
     static GLenum ShaderTypeFromString(const std::string& type) {
        ...
        ZED_CORE_ASSERT(false, "Unknown shader type!");
        return 0;
    }
    ...
    std::string OpenGLShader::ReadFile(const std::string &filePath) {
        ...
        if (in) {
            ...
            ZED_CORE_ERROR("Could not open file '{0}'", filePath);
        }
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string &source) {
        ...
        while(pos != std::string::npos) {
            ...
            ZED_CORE_ASSERT(eol != std::string::npos, "Syntax error")    // 没有换行，则后面没有逻辑，语法错误
            size_t begin = pos + typeTokenLength + 1;   // glsl第一行代码 +1表示跳过一个空格
            std::string type = source.substr(begin, eol - begin); //
            ZED_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified!")
    ...
    void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string> &shaderSources) {
        ...
        for (auto& kv : shaderSources) {
            ...
            if (isCompiled == GL_FALSE) {
                ...
                ZED_CORE_ERROR("{0}", infoLog.data());
                ZED_CORE_ASSERT(false, "Shader compilation failure!");
                break;
            }
        ...
        if (isLinked == GL_FALSE) {
            ...
            ZED_CORE_ERROR("{0}", infoLog.data());
            ZED_CORE_ASSERT(false, "Shader link failure!");
            return;
        }
        for (auto id : glShaderIDs) {
